...
{$check_quantity=ConfigLoader::byModule('shop')->check_quantity}
            ...
            <td class="compareItem{if !$product->isAvailable()} notAvaliable{/if}" data-id="{$product.id}">
                <div class="image{if count($imglist)>1} activelist{/if}">
                    {foreach $imglist as $image}
                        <img src="{$image->getUrl(188, 258)}" {if !$image@first}class="hidden"{/if}>
                    {/foreach}
                    {if !count($imglist)}
                        <img src="{$product->getImageStub()->getUrl(226,236)}">
                    {/if}
                </div>
                <div class="actions">
                    ...
                    <a href="{$router->getUrl('shop-front-cartpage', ["add" => $product.id])}" data-add-text="Добавлено" class="button addToCart noShowCart">В корзину</a><br>
