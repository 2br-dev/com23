    ...
    <form method="GET" class="filters" action="{urlmake f=null bfilter=null p=null}">
        ...
        {foreach $item.properties as $prop}
            ...
            {elseif $prop.type == 'bool'}
                <div class="filter fleft" style=" width: 300px; padding-left: 150px;">
                    ...
                    <!--<ul>
                        <li><input type="checkbox" name="f[{$prop.id}][]" {if is_array($filters[$prop.id]) && in_array($value, $filters[$prop.id])}checked{/if} value="1" id="cb_{$prop.id}_1"><label for="cb_{$prop.id}_1">есть</label></li>
                        <li><input type="checkbox" name="f[{$prop.id}][]" {if is_array($filters[$prop.id]) && in_array($value, $filters[$prop.id])}checked{/if} value="0" id="cb_{$prop.id}_2"><label for="cb_{$prop.id}_2">нет</label></li>
                        <li><input type="checkbox" name="f[{$prop.id}][]" {if is_array($filters[$prop.id]) && in_array($value, $filters[$prop.id])}checked{/if} value="" id="cb_{$prop.id}_3"><label for="cb_{$prop.id}_3">не важно</label></li>
                    </ul>-->
                </div>
            {else} {* string *}
		...
		<div class="cb"></div>
        <div class="buttons">            
            <a href="{urlmake f=null p=null bfilter=null}" class="button cleanFilter{if empty($filters) && empty($basefilters)} hidden{/if}">сбросить х</a>
