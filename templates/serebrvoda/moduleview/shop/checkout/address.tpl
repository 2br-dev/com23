...
<form method="POST" class="checkoutBox formStyle {$order.user_type|default:"authorized"}" id="order-form">
    ...
    {if $is_auth}
        ...
        <div class="formLine changeUser">
            <a href="{urlmake logout=true}">Сменить пользователя</a>
        </div>
    {else}
        ...
        <div class="address">
           ...
           {if count($address_list)>0}
                <div class="formLine lastAddress">
                    ...
                    <input type="radio" name="use_addr" value="{$address.id}" id="adr_{$address.id}" {if $order.use_addr == $address.id}checked{/if}><label for="adr_{$address.id}">{$address->getLineView()}</label><br>
                    {/foreach}
            ...
            {foreach $order->getCart()->getProductItems() as $data}
                {if $data.product->getMainDir()->name == 'Вода'}
        ...
        <div class="buttons">
            <input type="submit" class="button red-color" value="Далее">
        </div>
        <div class="infoLine">
