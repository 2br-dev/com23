...
<div class="sell-widget" id="sellWidget">
    <div class="widget-filters">
        ...
        <div class="dropdown">
            <a id="last-order-status" data-toggle="dropdown" class="widget-dropdown-handle">{if $orders=='success'}{t}завершенные заказы{/t}{elseif $orders=='all'}{t}все заказы{/t}{else}{t}мобильное приложение{/t}{/if} <i class="zmdi zmdi-chevron-down"></i></a>
            <ul class="dropdown-menu" aria-labelledby="last-order-status">
                ...
                <li {if $orders=='mobile'}class="act"{/if}><a data-update-url="{adminUrl mod_controller="shop-widget-sellchart" sellchart_range="{$range}" sellchart_orders="mobile" sellchart_show_type="{$show_type}"}" class="call-update">{t}Через мобильное приложение{/t}</a></li>
            </ul>
        </div>
