...
class CatalogProduct extends \Exchange\Model\Importers\AbstractImporter
{
    ...
    */
    private function insertProperty($product_id, $xml_id, $prop_id, $value)
    {
        ...
        if (in_array($type, \Catalog\Model\Orm\Property\Item::getListTypes())) {
            //Если это списковая характеристика, то добавляем возможное значение характеристики
            $val_list_id = \Catalog\Model\Orm\Property\ItemValue::getIdByValue($prop_id, $value_escaped);
        } else {
            $val_list_id = null;
        }
        ...
        $prop_link->val_int     = floatval($value);
    ...
    * @param \Catalog\Model\Orm\Product $product
    */
    private function importImages(\Catalog\Model\Orm\Product $product)
    {
        ...
        // Удаляем старые изображения (только то, у которого установлен парамет extra)
        /*
        $old_images = \RS\Orm\Request::make()
