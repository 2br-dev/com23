...
$(function() {
    ...
    var chb_bits = $('.check_all');
    chb_bits.each(function (){
        $(this).on('change', function () {
            var bit = $(this).val();
            var modules_bits = $(':input[value="'+bit+'"]');
            if($(this).is(':checked')){
                modules_bits.prop('checked', true);
            }else{
                modules_bits.prop('checked', false);
            }
        });
    });
    $('.access_bit').on('change', function () {
        if(!$(this).is(':checked')){
            var bit = $(this).val();
            $('.check_all[value="'+bit+'"]').prop('checked', false);
        }
    });
});
</script>
