...
*/
class AllOrdersCtrl extends \Shop\Controller\Admin\OrderCtrl
{
    function helperIndex()
    {
        ...
        $helper
            ->setTopHelp(t('Здесь отображаются оформленные пользователями и администраторами заказы. Эти заказы нельзя администрировать - они представленны только для информации'))
            ->setTopToolbar(new Toolbar\Element( array(
                'Items' => array(
                ))
            ))
            ->viewAsTable()
            ->setTopTitle(t('Все заказы - без возможности администрирования'))
            ...
            'Columns' => array(
                ...
                new TableType\Text('user_email', t('адрес электронной почты пользователя'), array('Sortable' => SORTABLE_BOTH, 'href' => $edit_href, 'hidden' => true) ),
    ...
    */
    function helperEdit()
    {
        ...
        $helper = new \RS\Controller\Admin\Helper\CrudCollection($this, $this->api, $this->url, array(
            'bottomToolbar' => $this->buttons(array()),
            'viewAs' => 'form'
        ));
