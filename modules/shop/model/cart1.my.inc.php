    ...
    */
    protected function addProductData($result, $use_currency)
    {
        ...
        foreach ($in_basket as $n => $item) {
           ...
           if ($product['site_id'] == 1){
               if ($product->getMainDir()->name == "Вода" || $product->getMainDir()->name == "Вода в анапе"){
                    if ($product['title'] != "Бутыль ПК 19 л"){ // Исключаем количесвто пустых бутылей
                        $amount_voda = $amount_voda + $amount;
                    }
               }
            } 

           // Если товар из категории Вода или Вода в анапе для сайта серебряная-вода.рф
            if ($product['site_id'] == 2){
               if ($product->getMainDir()->name == "Вода в таре 19 л." || $product->getMainDir()->name == "Вода в мелкой таре (меньше 19 л)"){
                    if ($product['title'] != "Бутылка ПК 19 л"){ // Исключаем количесвто пустых бутылей
                        $amount_voda = $amount_voda + $amount;
                    }
               }
            } 
        ...
        foreach($in_basket as $n => $item) {
            ...
            */
            ...
            $two_cost = $product->getPropertyValueByTitle('Цена от 2-х бутылей', 0);
            $four_cost = $product->getPropertyValueByTitle('Цена от 4-х бутылей', 0);
            if ($two_cost > 0 || $four_cost >0) {
                if ($amount >= 2 && $amount <4) $cost = $two_cost;
                if ($amount >=4) $cost = $four_cost;
            } */
            ...
            if ($product['site_id'] == 1){
                if ($product->getMainDir()->name == "Вода" || $product->getMainDir()->name == "Вода в анапе"){
                    if ($amount_voda >= 2 && $amount_voda < 4) {
                        $cost = $product->getCost(5, $item['cartitem']['offer'], false, true) != 0 ? $product->getCost(5, $item['cartitem']['offer'], false, true) : $product->getCost(null, $item['cartitem']['offer'], false, true);
                    }
                    if ($amount_voda >= 4) {
                        $cost = $product->getCost(6, $item['cartitem']['offer'], false, true) != 0 ? $product->getCost(6, $item['cartitem']['offer'], false, true) : $product->getCost(null, $item['cartitem']['offer'], false, true);
                    }
                }
            }   
            //Конец

            //Вставляем логику установки цен для товаров из категории Вода или Вода в анапе для сайта суребряная-вода.рф
            if ($product['site_id'] == 2){
                if ($product->getMainDir()->name == "Вода в таре 19 л." || $product->getMainDir()->name == "Вода в мелкой таре (меньше 19 л)"){
                    if ($amount_voda >= 2 && $amount_voda < 4) {
                        $cost = $product->getCost(7, $item['cartitem']['offer'], false, true) != 0 ? $product->getCost(7, $item['cartitem']['offer'], false, true) : $product->getCost(null, $item['cartitem']['offer'], false, true);
                    }
                    if ($amount_voda >= 4) {
                        $cost = $product->getCost(8, $item['cartitem']['offer'], false, true) != 0 ? $product->getCost(8, $item['cartitem']['offer'], false, true) : $product->getCost(null, $item['cartitem']['offer'], false, true);
                    }
                }
            }   
            //Конец
