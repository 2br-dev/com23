    ...
    _plotHover: function(event, pos, item) {
        if (item) {
            if (this.previousPoint != item.dataIndex) {
                ...
                var tooltipText = lang.t('Заказов ')+dateStr+': <strong>'+pointData.count+'</strong> <br\> ' + lang.t('На сумму') + ': <strong>'+this.numberFormat(pointData.total_cost,2,',',' ')+' '+this.chart.data('inlineData').currency+'</strong><br> ' + lang.t('Бутылок ')+dateStr+': <strong> '+pointData.bottles+'</strong> <br\> ';
