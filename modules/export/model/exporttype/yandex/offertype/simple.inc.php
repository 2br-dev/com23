...
class Simple extends CommonOfferType
{
    ...
    */
    function writeEspecialOfferTags(ExportProfile $profile, \XMLWriter $writer, Product $product, $offer_index, $use_htmlentity)
    {
        ...
        $writer->writeElement('name', $use_htmlentity ? htmlspecialchars_decode($name) : $name);
        if ($vendor = $product->getBrand()->title) {
            $writer->writeElement('vendor', $use_htmlentity ? htmlspecialchars_decode($vendor) : $vendor);
        }
        $writer->writeElement('description', $use_htmlentity ? htmlspecialchars_decode($product->short_description) : $product->short_description);
