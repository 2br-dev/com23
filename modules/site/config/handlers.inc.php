    ...
    public static function applyroute()
    {
        ...
        if ($redirect) {
            ...
            if ($protocol == 'http') {
                ...
                if (preg_match('/^\/storage/', $http->server('HTTP_HOST'))) $skipstorage = true;            
			
				 //Не производим redirect при обращении к некоторым маршрутам
                $skipistechroute = self::isTechRoute();
				
				if ($skipistechroute ||  $skipstorage) $skip = true;
    ...
    */
    ...
    private static function isTechRoute()
    {
        ...
        if ($route = \RS\Router\Manager::getCurrentRoute()) { 
            if (!\RS\Router\Manager::obj()->isAdminZone() || in_array($route->getId(), array(
                'main.admin',
