...
class Api
{
    ...
    function __construct($site_id = null, $map_type = null)
    {
        ...
        //Если есть партнёрские сайты и мы нанём, то добавим в нему ещё идентификатор
    ...
    */
    function generateSitemap()
    {
        ...
        if (\RS\Module\Manager::staticModuleExists('partnership') && \RS\Module\Manager::staticModuleEnabled('partnership')) {
             ...
             */
            if ($partner) {
                $protocol = \RS\Http\Request::commonInstance()->getProtocol(); //Добавим протокол
                $base_url = $protocol.'://'.rtrim($partner->getMainDomain(), '/');
            }
        }
